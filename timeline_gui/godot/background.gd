...
func _draw() -> void:
	...
	for i: int in range(num_lines):
		var year_x_pos: float = (-size.x * EXTRA_MARKER_ROOM) / 2 + offset + line_dist * i
		draw_line(Vector2(year_x_pos, -size.y / 2), Vector2(year_x_pos, size.y / 2), Color.DARK_GRAY)
		...
		if show_months:
			...
			for month: int in year.months():
				...
				draw_string(MONTH_FONT, Vector2(year_x_pos + month_dist * j - month_text_size.x/2, MainLine.vertical_offset+15.), month_text, HORIZONTAL_ALIGNMENT_CENTER, -1, 12)
				j += 1
